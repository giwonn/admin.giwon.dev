...
import { getVisitorStats, getPopularArticles, type PopularArticle } from "@/actions/analytics";
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const [statsData, articlesData, popularData] = await Promise.allSettled([
          getVisitorStats(),
        ...
        if (articlesData.status === "fulfilled") setRecentArticles(articlesData.value.content);
        if (popularData.status === "fulfilled") setPopularArticles(popularData.value);
      } finally {
        setIsLoading(false);
      }
    }
  ...
  return (
    <div className="p-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <StatCard label="오늘 방문자" value={stats?.today} isLoading={isLoading} />
        <StatCard label="어제 방문자" value={stats?.yesterday} isLoading={isLoading} />
        <StatCard label="전체 방문자" value={stats?.total} isLoading={isLoading} />
      </div>
